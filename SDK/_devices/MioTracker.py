...
class _OnlineEMGOffsetComp:
    ...
    """
    def __init__(self, n_channels=2, Fs=250, tau_ms=300.0):
        self.n = n_channels
        self.Fs = Fs
...
class MioTracker(BaseDevice):
    def __init__(self, transport: str = "websocket",  websocketuri:Optional[str] = None, port: Optional[str] = None, Fs: int = 250, baudrate: int = 115200, maxlen: int = 30,  trim_sec:float = 1.0) -> None:
        super().__init__()
    ...
    def connect(self) -> bool:
        if self._transport == "serial":
            ...
            self.websocketuri = "ws://miotracker.local/start" if not self.websocketuri else self.websocketuri
    ...
    def disconnect(self,port: Optional[str] = None) -> bool:
        if self._transport == "serial":
            if self._ser and self._ser.is_open:
                print(f"[MioTracker] Cerrando puerto serial {self._ser.port}...")
                self._ser.close()
                self._ser = None
            ...
            if self._ws_client:
                print("[MioTracker] Cerrando conexión WebSocket...")
    ...
    def start(self) -> None:
        if self._transport == "serial":
            if not self._ser or not self._ser.is_open:
                print("Puerto serial no abierto. Llama a connect() primero.")
            ...
            print("[MioTracker] Adquisición iniciada.")
        elif self._transport == "websocket":
            if not self._ws_client:
                print("Socket no abierto. Llama a connect() primero.")
    ...
    def stop(self) -> None:
        if self._transport == "serial":
            ...
            self._usb_thread = None
            print("[MioTracker] Adquisición detenida.")
        elif self._transport == "websocket":
            if self._ws_client:
    ...
    def get_emg_df(self,onlyraw = False) -> pd.DataFrame:
        ...
        if self.trim_sec and not df.empty:
            ...
            df = df[df.index >= t0] 
        if onlyraw:
            # Solo columna ch0 + Timestamp como columna normal
        ...
        try:
            ...
            with self._lock:
                ...
                if not hasattr(self, "_ema"):
                    self._ema = _OnlineEMGOffsetComp(n_channels=1, Fs=self.Fs, tau_ms=300)
...
if __name__ == "__main__":
    try:
        ...
        print(f"Error al conectar el MioTracker: {e}")
